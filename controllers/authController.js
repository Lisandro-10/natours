...
const createSendToken = (user, statusCode, res) => {
	...
	const cookieOptions = {
		expires: new Date(
			Date.now() + process.env.JWT_COOKIE_EXPIRES_IN * 68400000
		), //24*60*60*1000
		httpOnly: true,
	}
...
exports.protect = catchAsync(async (req, res, next) => {
	...
	if (!currentUser) {
		return next(
			new AppError(
				'The token user belonging to this token does not longer exist!',
				401
			)
		)
	}
...
exports.isLoggedIn = async (req, res, next) => {
	if (req.cookies.jwt) {
		try {
			...
			//3) if current user changed password after the token was issued
			if (currentUser.changedPasswordAfter(decoded.iat)) {
				return next()
			}
			// THERE IS A LOGGED INT USER
			res.locals.user = currentUser
			return next()
		} catch (err) {
			return next()
...
exports.updatePassword = catchAsync(async (req, res, next) => {
	...
	if (!user) {
		return next(new AppError('User does not longer exist.', 401))
	}
	//2) Check password
	if (
